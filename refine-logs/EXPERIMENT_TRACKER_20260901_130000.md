...
> Updated：2026-09-01 13:00 +08:00
>
...
| INFRA-CAPTURE | M1 | Android capture finalization gate | fail-closed offline finalizer + artifact-refinalizing coverage audit | synthetic test bundles only | 24 finalizer tests；typed Java/Python canonical hash；package/focus/path/value-domain recheck；5-group/3-template/3-strata contract | MUST | DONE | tooling only；adb available but no connected device/emulator/appium；API 34 sensitive bit not independently recomputable；paper_result_eligible=false |
...
| INFRA-K50-GATE | M4 | formal superiority finalizer | MG-PU K50 vs seeded random-K50 | formal paired report pending | adjudicated VPMA；5 groups/3 templates；10k cluster bootstrap；coverage/hallucination/actual-budget/selected-overlap gates | MUST | DONE | 11 tests；budget-matched vs item-matched interpretation explicit；no formal report/result；Holm/BH/Pareto and upstream formal gold/B1 remain blockers；paper_result_eligible=false |
| MGPU-002 | M4 | frozen main run | MG-PU vs seeded random-K at K50 | test | primary paired comparison | MUST | TODO | K=ceil(0.50×N) visual calls；test once；formal finalizer ready, but formal item/gold schema, B1, ledger generation and paired report remain pending |
