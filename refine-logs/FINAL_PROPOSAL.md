# Research Proposal：面向视障人士的可操作性缺口感知弹窗恢复

> 状态：当前最佳本地版本；已经过同模型家族预审，未经外部跨模型审阅；`acceptance_status: provisional`
...
- **Bottom-line problem**：对依赖 TalkBack、VoiceOver 等屏幕阅读器操作移动设备的视障人士，普通、低风险且可合法退出的模态界面可能阻断原任务；平台暴露的结构化可访问性表示又可能遗漏、合并或异构呈现关键操作入口，使仅依赖可访问性树的处理器留下可测的残余失败。
...
- 目标人群已冻结为使用 TalkBack、VoiceOver 等屏幕阅读器的视障人士，主要包括盲人和低视力用户。
- 若没有目标用户参与，技术实验只能证明辅助型 GUI Agent 的能力，不能直接声称已改善视障人士的真实体验。
...
- **One-sentence thesis**：Actionability-Gap-Gated Recovery 先判断结构化表示是否提供语义明确、可执行且 owner/context 正确的低风险退出路径；只有在缺失、合并、歧义或不可操作时才按需调用冻结的视觉 grounding，并以 `D ∧ C_a11y ∧ T` 证书判定面向视障用户的恢复成功。
...
- **Supporting contribution**：构建并计划公开一个成对保存结构化表示、截图、合法动作集合、`VTR-tech` 与 `A-VTR` 回证的 Android/iOS benchmark；公开发布仍取决于授权、隐私、版权与复现检查。
...
协议动作 > 元素动作 > grounded coordinate tap
        ↓
D（弹窗解除）∧ C_a11y（owner/context、原目标与屏幕阅读器焦点恢复）∧ T（任务后置条件成立）
        ↓
...
6. 执行后获取新树、新截图、owner/context、屏幕阅读器焦点/朗读证据与任务探针，分别计算 `VTR-tech` 和 `A-VTR`。
7. 验证失败时最多尝试一个预注册替代动作，随后 handoff。

### Verification Contract

- **D — Dismissal**：弹窗的视觉标识与语义窗口／节点均不再存在；只满足其中一项不足以计成功。
- **C_tech — Technical context recovery**：前景 package/bundle/window/context 正确，原被阻断目标重新可见且可操作。
- **C_a11y — Accessible context recovery**：满足 C_tech，且屏幕阅读器焦点回到原目标或合法后继目标；若朗读状态可观测，下一段朗读与恢复后的任务一致。
- **T — Task postcondition**：原任务的领域后置条件成立；若任务没有可观测后置条件，该 episode 不进入主 A-VTR/VTR-tech 分母或单独报告为 unverifiable。
...
| D 成立但 C_tech/C_a11y/T 失败 | 完整 verification trace | 重观察或恢复原任务，不计对应层级成功 |
...
- **Metric**：A-VTR（主）、VTR-tech（次）、valid-close-item recall、False Intervention、Harmful Action、abstention-coverage；按平台、App、缺口类型报告。
- **Expected evidence**：本方法在结构暴露缺口子集和总体集上提高 A-VTR；若焦点不可机器观测，先报告 VTR-tech 与 A-VTR 可观测子集，且不以更高错误动作率换取覆盖。

### Claim 2：按需视觉比始终视觉更有效率、更稳健

- **Minimal experiment**：固定 VLM、提示、候选和推理预算，对比 gated 与 always-on vision。
- **Ablations**：去掉视觉；去掉门控；门控只检查空树；去掉 owner consistency；去掉屏幕阅读器焦点检查；只验证画面变化；只验证 D。
- **Metric**：视觉调用率、端到端延迟、VLM cost、False Intervention、A-VTR/VTR-tech 差值。
- **Expected evidence**：gated 方法在 A-VTR/VTR-tech 不显著下降的前提下降低视觉调用、延迟与误干预；若 always-on 明显更优，则门控不构成贡献。

### Pre-registered Falsification / Stop Conditions

- tree-only 与本方法的 A-VTR/VTR-tech 置信区间重叠且成本更低：视觉机制不成立。
- naive always-on fusion 在等预算下稳定优于门控：门控不是贡献。
- 增益在 App／模板隔离切分后消失：存在数据泄漏或过拟合。
- iOS 没有完成真实 `D ∧ C_tech ∧ T`：不能声称跨 Android/iOS 技术验证；没有 `D ∧ C_a11y ∧ T` 或真实用户实验时，不能声称跨平台视障用户恢复。
...
2. 我们计划构建一个 Android/iOS 配对 benchmark，记录结构化表示、像素、合法动作集合及 VTR-tech/A-VTR 回证。
...
- **Must-prove claims**：非空结构化缺口的可检测性；gated vision 对 A-VTR/VTR-tech 的增益；`D ∧ C_a11y ∧ T` 相对弱回证的纠错价值。
...
- **Timeline**：先完成小规模跨平台可行性 pilot、冻结 schema、VTR-tech 与 A-VTR oracle，再做功效分析和正式数据规模设计。
